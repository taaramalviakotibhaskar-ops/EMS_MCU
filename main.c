...
void TIM2_IRQHandler(void)
{
    ...
    if (blinking)
    {
        ...
        // LED2 on PB0 — rate set by potentiometer
...
{
    adcValue = ADC_DR;              // read potentiometer value (0-4095)
    ADC_ISR &= ~(1 << 2);          // clear conversion flag

    // linear interpolation: 0 = 100ms (5Hz), 4095 = 500ms (1Hz)
    blink_delay = 100 + (adcValue * 400) / 4095;

    led2_count = 0;                 // reset so new delay applies cleanly
    ADC_CR |= (1 << 2);            // start next conversion
}
...
int main(void)
{
    ...
    // GPIO — PA5 as output (LED1)
    GPIOA_MODER &= ~(3 << 10);
    GPIOA_MODER |=  (1 << 10);

    // GPIO — PB0 as output (LED2)
    GPIOB_MODER &= ~(3 << 0);
    GPIOB_MODER |=  (1 << 0);

    // GPIO — PC13 as input (button)
    GPIOC_MODER &= ~(3 << 26);

    // GPIO — PA0 as analog input (potentiometer)
    GPIOA_MODER |=  (3 << 0);
    ...
    ADC_IER   |= (1 << 2);       // enable end of conversion interrupt
    ADC_CR    |= (1 << 0);       // enable ADC
    while (!(ADC_ISR & (1 << 0))); // wait for ADC ready
    NVIC_SetPriority(ADC1_COMP_IRQn, 2);
    NVIC_EnableIRQ(ADC1_COMP_IRQn);
    ADC_CR |= (1 << 2);           // start first conversion
